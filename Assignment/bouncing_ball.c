...
void bounceBall()
{
    if (direction == 0) { // Ball moving towards bottom-left
        center[0] = center[0] - movement;
        center[1] = center[1] - movement;
        if (center[0] <= -0.5f || center[1] <= -0.5f) { // Check if ball reaches left or bottom edge
            direction = 1; // Change direction to move towards top-left
        }
    } else if (direction == 1) { // Ball moving towards top-left
        center[0] = center[0] - movement;
        center[1] = center[1] + movement;
        if (center[0] <= -0.5f || center[1] >= 0.5f) { // Check if ball reaches left or top edge
            direction = 2; // Change direction to move towards top-right
        }
    } else if (direction == 2) { // Ball moving towards top-right
        center[0] = center[0] + movement;
        center[1] = center[1] + movement;
        if (center[0] >= 0.5f || center[1] >= 0.5f) { // Check if ball reaches right or top edge
            direction = 3; // Change direction to move towards bottom-right
        }
    } else if (direction == 3) { // Ball moving towards bottom-right
        center[0] = center[0] + movement;
        center[1] = center[1] - movement;
        if (center[0] >= 0.5f || center[1] <= -0.5f) { // Check if ball reaches right or bottom edge
